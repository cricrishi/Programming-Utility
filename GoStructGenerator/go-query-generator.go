...
func main() {
	...
	generateQuery(*table_name)

}

//function for generating golang struct
func generateQuery(table_name string) {
	err, columns := getColumnsForQuery(table_name)
	if err != nil {
		return
	}
	...
	fmt.Print(" from " + table_name)
}

// Function for fetching schema definition of passed table
func getColumnsForQuery(table string) (errr error, columns []column) {
	rows, err := db.Query(`
